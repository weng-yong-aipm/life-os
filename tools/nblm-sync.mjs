 ...
 *
 * HONEST SCOPE: NotebookLM has no public API and the browser-automation MCP
 * cannot validate a session for this account, so the *upload into NotebookLM*
 * stays a manual 10-second drag. What this DOES automate, reliably and with no
 * Google login:
 *   1. Regenerate ~/life-os-vault/notebooklm/learnings-all.md from Supabase so
 *      the NotebookLM source is always current (the "auto-push" — a fresh,
 *      upload-ready source file).
 ...
 *      there's fresh material to drag in (the "watchdog").
 *
...
async function main() {
  ...
  // Regenerate the upload-ready bundle.
  mkdirSync(OUT_DIR, { recursive: true });
  writeFileSync(BUNDLE, buildBundle(rows, now));

  // Watchdog: count new items since the last sync.
  ...
  // Surface a status note in the Obsidian vault (this is the notification).
  const lines = [
  ...
  if (newCount > 0) {
    lines.push(
      `## 🆕 ${newCount} new item(s) ready to upload`,
      '',
      'Drag `learnings-all.md` into your NotebookLM notebook (Add source → Upload) to refresh it.',
      '',
      '_(NotebookLM has no API and the browser-automation login can\'t validate this account,',
      'so this last step is manual — everything up to it is automated.)_',
      '',
      '### What\'s new',
      '',
